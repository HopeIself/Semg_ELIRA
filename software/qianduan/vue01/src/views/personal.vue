      ...
      <form @submit.prevent="submitForm" class="form-grid">
        ...
        <div class="form-group">
          ...
          <ul v-if="doctorSuggestions.length" class="suggestion-list">
            <li
              v-for="doc in doctorSuggestions"
              :key="doc.id"
        ...
        <div class="button-group">
          ...
          <!-- 新按钮，显示当前医生 -->
          <button
            class="show-doctor-button"
            type="button"
            @click="showCurrentDoctor"
          >
            显示当前医生
          </button>
        </div>
  ...
  methods: {
    ...
    async searchDoctors() {
      ...
      try {
        const res = await axios.get(
          "http://115.190.134.66:5000/api/search_doctors",
    ...
    async submitForm() {
      ...
      try {
        const resp = await axios.post(
          "http://115.190.134.66:5000/api/feature-lookup",
          payload
        );
        ...
        if (resp.data.success) {
          ...
          if (resp.data.message && resp.data.message.includes("该医生已存在")) {
            this.errorMessage = `该医生已存在: ${resp.data.doctor_name}`;
          } else {
    ...
    async showCurrentDoctor() {
      try {
        // 向后端发送用户的 ID 获取医生信息
        const resp = await axios.post(
          "http://115.190.134.66:5000/api/get_doctor",
          { id: this.user.id }
        );
